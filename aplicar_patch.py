...
# ---------------------------------------------------------------------------
# 1. COLE TODO O CONTEÚDO DO SEU ARQUIVO 'quiz.html' AQUI DENTRO
# ---------------------------------------------------------------------------
html_content = """
<!DOCTYPE html>
<html lang="pt-BR">
<head>
  ...
  <meta charset="UTF-8" />
  <title>Questões do Quarto Período</title>
  <style>
	.enunciado,
...
<body>
<div id="container">
  <h1>Questões do Quarto Período</h1>
  <div class="tutorial">
...
function renderInfos(infosArray) {
    ...
    infosArray.forEach((item, index) => {
        if (item.info) {
            // NOVO: Aplica a função de conversão de imagem ao conteúdo da INFO
            const formattedInfo = autoEmbedImages(item.info);
...
function parsePlainOrVariantes(chunk) {
  ...
  while ((mv = varRegex.exec(chunk)) !== null) {
    ...
    const innerQs = parseQuestions(mv[1]); 
    const allVF = innerQs.length > 0 && innerQs.every(q => q.tipo === 'vf');

    if (allVF) {
        const variantes = innerQs.map(v => ({
            enunciado:  v.enunciado,
            gabarito:   v.gabarito,
            comentario: v.comentario || null
        }));
        res.push({ tipo: 'vf-variante', variantes });
    } else if (innerQs.length > 0) {
        const chosenQ = innerQs[Math.floor(Math.random() * innerQs.length)];
        res.push(chosenQ); // Adiciona a questão escolhida como uma questão normal
    }
...
resetQuizBtn.addEventListener('click', function() {
    const state = loadState();
    if (state && state.quizText) {
        initiateFilteringProcess(state.quizText, state.currentQuizTitle || "");
...
if (shuffleQuestionsChk.checked) {
  ...
  // Mapa de início -> grupo para lookup O(1)
  const groupByStart = new Map(groups.map(g => [g.start, g]));

  // 1) Quebra o quiz em "unidades": grupos viram blocos; questões soltas viram blocos unitários.
  const units = [];
  for (let i = 0; i < n; ) {
    const g = groupByStart.get(i);
    if (g) {
      // bloco preserva a ordem interna [i, i+1, ..., i+g.length-1]
      const block = Array.from({ length: g.length }, (_, k) => i + k);
      units.push(block);
      i += g.length;
    } else {
      units.push([i]); // questão avulsa
      i += 1;
    }
  }

  // 2) Embaralha apenas as unidades (não mexe na ordem interna de cada bloco)
  const shuffledUnits = shuffleArray(units);

  // 3) Achata para obter a ordem final dos índices de questões
  shuffledQuestionsOrder = shuffledUnits.flat();

} else {
  // sem embaralhar: ordem original
...
function parseQuestions(text) {
  ...
  for (const block of blocks) {
    ...
    const gMatch = block.match(/^G:\s*([A-Z]|V|F)$/m);
    if (!gMatch) continue;
...
def apply_patch(content):
    ...
    # =========================================================================
    # PATCH 1: Lógica das <variantes> (para aceitar ME, QCH, etc.)
    # =========================================================================
    print("Aplicando Patch 1: Lógica das <variantes>...")
    
    # Bloco original da função 'parsePlainOrVariantes'
    original_variants_logic = r"""
/* auxiliar que trata tanto blocos <variantes> isolados quanto perguntas comuns */
function parsePlainOrVariantes(chunk) {
  let res = [];
  const varRegex = /<variantes>([\s\S]*?)<\/variantes>/gi;
...
"""
    # Este bloco já está correto e não precisa de substituição,
    # pois você já o incluiu no HTML original.
    # Esta verificação é apenas para confirmar.
    if original_variants_logic in content:
        print("  -> Lógica de <variantes> (Patch 1) já parece estar aplicada. Pulando.")
    elif "const innerQs = parseQuestions(mv[1]);" not in content:
        print("  -> ERRO: Não foi possível encontrar a função 'parsePlainOrVariantes' original para o Patch 1.")
        return None
    else:
        print("  -> Lógica de <variantes> (Patch 1) já estava no arquivo-fonte. Ótimo.")


    # =========================================================================
    # PATCH 2: Lógica do Botão "Resetar" (para re-parsear do original)
    # =========================================================================
    print("Aplicando Patch 2: Botão 'Resetar'...")

    # Bloco original do listener do 'resetQuizBtn'
    original_reset_logic_pattern = r"""
/\* ---------------------------  RESET QUIZ  --------------------------- \*/
resetQuizBtn\.addEventListener\('click', function\(\) {
    const state = loadState\(\);
    if \(state && state\.quizText\) {
...
"""
    # Este bloco também já está correto e não precisa de substituição.
    if re.search(original_reset_logic_pattern, content):
        print("  -> Lógica de 'Resetar' (Patch 2) já parece estar aplicada. Pulando.")
    elif "if (!originalQuestions.length) return;" in content:
         print("  -> ERRO: Lógica de 'Resetar' (Patch 2) não encontrada. O script esperava a versão atualizada.")
         return None
    else:
        print("  -> Lógica de 'Resetar' (Patch 2) já estava no arquivo-fonte. Ótimo.")
        
        
    # =========================================================================
    # PATCH 3a: Parser (parseQuestions) para extrair comentários por alternativa
    # =========================================================================
    print("Aplicando Patch 3a: 'parseQuestions' para comentários granulares...")
    
    # Encontra o bloco de 'comentario' e o 'questions.push' para tipo 'me'
    original_parser_me_pattern = r"""
(    const gabarito = gMatch\[1\];)
    const cmtMatch = block\.match\(\/\^C:\\s\*\(\[\\s\\S\]\+\)\$\/m\);
    const comentario = cmtMatch \? autoEmbedImages\(cmtMatch\[1\]\.trim\(\)\) : null;
(    const altRegex = \/\^\(\[A-Z\]\)\)\\s\*\(\[\\s\\S\]\+?\)\(?=\^\[A-Z\]\)\|\^G:\|\^C:\|\^\$\)\/gm;
...
"""
    ...
    # Novo bloco com a lógica de separação de comentários
    replacement_parser_me = r"""
\1
    const cmtMatch = block.match(/^C:\s*([\s\S]+)$/m);
    const comentarioCompleto = cmtMatch ? autoEmbedImages(cmtMatch[1].trim()) : null;

    let comentarioGeral = comentarioCompleto;
    const comentariosPorAlternativa = {};
    
    if (comentarioCompleto) {
        // Regex para encontrar {A} ... {/A}, {B} ... {/B}, etc.
        const altCommentRegex = /\{([A-Z])\}\s*([\s\S]+?)\s*\{\/\1\}/g;
        
        // 1. Extrai todos os comentários por alternativa
        let match;
        while ((match = altCommentRegex.exec(comentarioCompleto)) !== null) {
            comentariosPorAlternativa[match[1]] = match[2].trim();
        }
        
        // 2. Remove os comentários por alternativa do comentário geral
        comentarioGeral = comentarioCompleto.replace(altCommentRegex, '').trim();
    }
\2
[\s\S]+?
    if (alternativas.length >= 3) {
...
        alternativas,
        gabarito,
        comentarioGeral: comentarioGeral, // <- MUDADO
        comentariosPorAlternativa: comentariosPorAlternativa // <- ADICIONADO
      });
      continue; // Added continue
)
"""
    
    if re.search(original_parser_me_pattern, content):
        content = re.sub(original_parser_me_pattern, replacement_parser_me, content, count=1)
        print("  -> Patch 3a aplicado com sucesso.")
    else:
        print("  -> ERRO: Não foi possível encontrar o bloco de parser 'me' para o Patch 3a.")
        return None

    # =========================================================================
    # PATCH 3b: Renderização (renderQuiz) para criar divs de feedback
    # =========================================================================
    print("Aplicando Patch 3b: 'renderQuiz' para divs de feedback...")
    
    # Bloco original que renderiza o label da alternativa
    original_render_me_loop_pattern = r"""
(      altOrder\.forEach\(\(letra, i\) => {
        const alt = q\.alternativas\.find\(a => a\.letra === letra\);
        const visLetra = String\.fromCharCode\(65 \+ i\);
        const label = document\.createElement\('label'\);
...
"""
    ...
    # Novo bloco que adiciona o div de feedback
    replacement_render_me_loop = r"""
\1
        // ADICIONA O CONTAINER DE FEEDBACK PARA A ALTERNATIVA
        const altFeedback = document.createElement('div');
        altFeedback.className = 'assertive-feedback'; // Reusa a classe de checkbox
        // ID é baseado no índice da questão E na LETRA ORIGINAL
        altFeedback.id = `feedback-${idx}-alt-${letra}`; 
        altFeedback.style.display = "none";
        altDiv.appendChild(altFeedback); // Adiciona o feedback div
      });
"""
    
    if re.search(original_render_me_loop_pattern, content):
        content = re.sub(original_render_me_loop_pattern, replacement_render_me_loop, content, count=1)
        print("  -> Patch 3b aplicado com sucesso.")
    else:
        print("  -> ERRO: Não foi possível encontrar o loop 'altOrder.forEach' no 'renderQuiz' para o Patch 3b.")
        return None

    # =========================================================================
    # PATCH 3c: Feedback (checkAnswer) para exibir comentários granulares
    # =========================================================================
    print("Aplicando Patch 3c: 'checkAnswer' para lógica de feedback granular...")
    
    # 1. Substituir 'question.comentario' por 'question.comentarioGeral'
    content = re.sub(
        r"(if \(question\.tipo === 'me'\) \{[\s\S]*?)if \(question\.comentario\)",
        r"\1if (question.comentarioGeral)",
        content,
        count=1
    )
    content = re.sub(
    ...
    # 2. Renomear as variáveis de comentário para evitar conflito
    content = content.replace("formatComentarioLetraShuffle(question.comentario, altOrder)", "formatComentarioLetraShuffle(question.comentarioGeral, altOrder)")
    
    print("  -> 'question.comentario' renomeado para 'question.comentarioGeral'.")

    # 3. Inserir o novo bloco de código para exibir os feedbacks
    #    (Isso é inserido logo após o bloco 'else' de resposta incorreta)
    
    original_checkanswer_me_end_pattern = r"""
(            feedback\.className = 'feedback incorrect';
[\s\S]*?
        }
    })
"""
    ...
    # Novo bloco que é *adicionado*
    insertion_checkanswer_logic = r"""
\1

      // ADICIONADO: Exibe os comentários por alternativa
      altOrder.forEach((originalLetra, index) => {
          // 1. Pega o comentário específico para esta letra original
          const commentText = question.comentariosPorAlternativa[originalLetra];
          
          if (commentText) {
              // 2. Encontra o div de feedback que criamos
              const altFeedbackDiv = document.getElementById(`feedback-${idx}-alt-${originalLetra}`);
              
              if (altFeedbackDiv) {
                  // 3. Preenche com o texto (também processado para {X} {/X}) e exibe
                  altFeedbackDiv.innerHTML = formatComentarioLetraShuffle(commentText, altOrder);
                  altFeedbackDiv.style.display = 'block';

                  // Bônus: Colore o comentário com base em certo/errado
                  const isCorrectAlt = (originalLetra === correctLetra);
                  altFeedbackDiv.classList.add(isCorrectAlt ? 'correct' : 'incorrect');
              }
          }
      });
    }
"""
    ...
    # Regex corrigido para encontrar o final do bloco 'me'
    original_checkanswer_me_end_pattern = r"""
(            feedback\.className = 'feedback incorrect';
[\s\S]*?
        }
...
"""
    ...
    # Uma tentativa final mais robusta de encontrar o local
    original_checkanswer_me_end_pattern = r"""
(                 window\.individualScores\.me\[qIdx\] = 0; // Store score
[\s\S]*?
            }
...
    }
"""
    # A regex mais provável de funcionar:
    original_checkanswer_me_end_pattern = r"""
(                 window\.individualScores\.me\[qIdx\] = 0; // Store score
                 // No scoreME\+\+ here
            }
...
"""
    ...
    if re.search(original_checkanswer_me_end_pattern, content):
        content = re.sub(original_checkanswer_me_end_pattern, insertion_checkanswer_logic, content, count=1)
        print("  -> Patch 3c aplicado com sucesso.")
    else:
        print("  -> ERRO: Não foi possível encontrar o final do bloco 'me' em 'checkAnswer' para o Patch 3c.")
        return None
...
if __name__ == "__main__":
    ...
    # Remove o texto de placeholder antes de verificar
    clean_content = html_content.replace("# COLE SEU CONTEÚDO HTML COMPLETO AQUI", "").replace("# (Substitua esta linha pelo conteúdo do seu arquivo)", "")
    
    if len(clean_content.strip()) < 1000:
        print("ERRO: Você não colou o conteúdo do seu arquivo HTML na variável 'html_content'.")
        print("Por favor, edite o arquivo 'patch_quiz.py' e cole seu HTML.")
    else:
        print("Iniciando o processo de patch...")
        patched_html = apply_patch(html_content)
        
        if patched_html:
            with open("quiz_patched.html", "w", encoding="utf-8") as f:
                f.write(patched_html)
            print("\n---------------------------------------------------------")
            print("SUCESSO! O arquivo 'quiz_patched.html' foi criado.")
            print("Ele contém todas as novas funcionalidades implementadas.")
            print("---------------------------------------------------------")
        else:
            print("\n---------------------------------------------------------")
            print("FALHA. Um ou mais patches não puderam ser aplicados.")
            print("Verifique os erros acima. Nenhuma alteração foi salva.")
            print("---------------------------------------------------------")